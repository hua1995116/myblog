name: 《秋风的笔记第三期》彻底理清 AMD,CommonJS,CDM,UMD,ES6
tag: 模块化
description: 
...
// a.js
```
此处需要注意，在 `深入理解 ES6 模块机制` 中结果有所不一致，是由于它只通过 babel-node 去执行，通过转化为 `CommonJS` 的代码去执行，从而丢失了这个 `export` 提升的特性。而在浏览器中则是能获取到所
...
1. 分析 commonjs - plugin
2. 分析 rollup 如何工作
3. snowpack 入门，提高10倍打包速度。
...
1. 依托于 ES Modules, 对于一些包含陈旧的 CommonJS 包，需要额外地处理
2. 无法做到 Tree shaking
