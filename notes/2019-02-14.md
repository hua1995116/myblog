name: 急速 debug 指南 （Node - webpack插件,babel插件,vue源码篇）
tag: Chrome
description: 
...
示例代码地址： https://github.com/hua1995116/debug

# node 调试

在 node 基础调试中，会给出 chrome 和 vscode 两种调试方式，可以根据自己的喜好来进行选择调试，后面所有示例都以 vscode 为例子。
...
第二步，回到我们的 `index.js` 代码中，打上一个断点，vscode 打断点为，在某一行代码前点击，会出现一个红点点。
...
# webpack插件、babel插件
相信如果你在写插件的时候通过 `console.log` 的形式会让你的插件开发变得分外的困难和头疼。还有就是看别人写的插件，如果你只会 `console.log`,那么也会令原本简单的事情变得非常复杂。
...
如果再到某个细节 ，你可以继续写入 debugger。

# 结尾

看完了本教程，希望对大家有所帮助~，如果有更好地调试方式，欢迎评论来秀一波操作。