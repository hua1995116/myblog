name: 函数式编程看 React Hooks （二）事件绑定副作用深度剖析
tag: javascript,react
description: 

# 前言

本教程不讲解 React Hooks 的源码，只用最简单的方式来揭示 React Hooks 的原理和思想。 （我希望你看本文时，已经看过了上面一篇文章，因为本文会基于你已经了解 部分 hooks 本质的前提下而展开的。例如你懂得 hooks 维护的状态其实是一个由闭包提供的。）
...
我们尝试一下，嗯，看似完美地解决了。但是我们会发现，哇，为什么重新渲染了那么多次？还记得我们 上一篇文章中，介绍 dep 比较的原理吗？直接对值进行的比较。也就是意味着函数的话，就是地址进行比较，显然，每次创建的函数地址都是不同的。
...
现在只有当我们的 `isTag` 和 `count` 改变的时候才会更新我们的 `onMouseMove`。


现在我们已经完美地解决了我们的问题，并且讲解了 hooks 的一些本质，为什么这么做的原理？我们再打上日志，来感受下，整个 hooks 的运行过程吧。


运行 useeffect 中的 callback （绑定事件等）
...
运行 useeffect 中的 callback return （解绑事件）

运行 useeffect 中的 callback （绑定事件等）

当然我们也可以通过 console 来表现，整个一次渲染的过程
...
2. 一定要添加对的依赖。否则也会出现性能问题。
   
看法：
1.在某种程度上用性能来换取函数式编程的规范，不是所有人都对 hooks 原理了如指掌（除了是看过我这个系列教程的）。因此写出不理想的代码是非常常见的。
2.非常佩服react团队的精神，为了这个方向，能想出这样的解决方法。毕竟是 浏览器 与 react 的编程模式是不一样。他进行了最大程度上的融合。
